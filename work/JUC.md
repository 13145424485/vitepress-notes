...
线程是操作系统调度的执行单位，协程是**用户态自己管理的更轻量执行单位**。

线程切换需要操作系统介入，要保存和恢复寄存器、程序计数器、栈这些上下文，还会涉及内核态和用户态切换，所以成本比较高；

协程切换一般发生在用户态，由运行时或者框架自己调度，不需要频繁陷入内核，所以开销更小。

协程切换更快，因为它更轻、上下文更小、调度更靠近应用层。**线程是系统管，协程是应用自己管；线程切换重，协程切换轻。**

**协程适合高并发 IO 场景，线程更适合真正吃 CPU 的并行计算场景。**
...
内核态切换就是程序从**用户态进入内核态，再从内核态返回用户态的过程**。一般发生在系统调用、异常、中断这些场景。它之所以有成本，是因为要切换执行权限、保存现场、恢复上下文，所以频繁切换会影响性能。很多高性能优化，本质上都是在减少不必要的内核态切换。
...
核心线程数 = 最大线程数，线程数固定。任务满了进入无界队列等待，无法扩容。用于 已知并发压力、对并发度有严格限制的场景。程数量就是固定的，就算任务数超过线程数，线程池也不会再创建更多的线程来处理任务，而是会把超出线程处理能力的任务放到任务队列中进行等待。而且就算任务队列满了，到了本该继续增加线程数的时候，由于它的最大线程数和核心线程数是一样的，所以也无法再增加新的线程了。

**CachedThreadPool（缓存线程池）**

核心线程数为 0，最大线程数`Integer.MAX_VALUE`（无限）。空闲线程超时回收，当然它也有一个用于存储提交任务的队列，但这个队列是 SynchronousQueue，队列的容量为 0，实际不存储任何任务，它只负责对任务进行中转和传递，所以效率比较高。用于任务数不确定、执行时间短的高并发场景。线程数无限，大量任务涌入可能导致问题。

**SingleThreadExecutor（单线程线程池）**

池中只有一个线程。异常终止会自动新建线程接替。必须保证任务**串行执行**、顺序执行的场景（如日志打印、顺序作业）。
...
阻塞队列是自带阻塞特性的线程安全队列，核心用于生产者 - 消费者模式，Java 里常用的有数组实现的 `ArrayBlockingQueue`、链表实现的 `LinkedBlockingQueue`，还有支持优先级、延迟、直接传递的特殊队列，能简化并发同步逻辑，提升多线程协作效率。

------

**阻塞队列就是支持 “阻塞式入队 / 出队” 的线程安全队列**，它自带两个关键特性：

- 当队列为空时，消费者线程获取元素会被阻塞，直到队列有数据；
- 当队列满时，生产者线程添加元素会被阻塞，直到队列有空位。

它最经典的用法就是**生产者 - 消费者模式**，生产者往队列里塞数据，消费者从队列里取数据，队列自动帮你处理线程等待和通知，不用自己写复杂的同步逻辑。
...
| 队列                    | 底层结构      | 特点                                                         | 适用场景                             |
| ----------------------- | ------------- | ------------------------------------------------------------ | ------------------------------------ |
| `ArrayBlockingQueue`    | 数组          | 有界队列，创建时必须指定固定大小，使用一把锁控制并发         | 固定容量的生产者 - 消费者场景        |
| `LinkedBlockingQueue`   | 链表          | 默认无界（最大到`Integer.MAX_VALUE`），也可指定大小，读写用两把锁，并发性能更高 | 高并发、队列容量可伸缩的场景         |
| `PriorityBlockingQueue` | 数组 + 二叉堆 | 无界阻塞队列，元素按优先级排序                               | 按优先级处理任务的场景（如任务调度） |
| `DelayQueue`            | 优先级队列    | 无界队列，元素只有在延迟期满时才能被取出                     | 定时任务、超时处理场景               |
| `SynchronousQueue`      | 无存储结构    | 不存储元素，生产者和消费者必须配对出现                       | 直接传递任务的场景（如线程池）       |

## 讲一下线程安全List？

### 1. `Collections.synchronizedList`

- **原理**：对传入的普通 List（如 `ArrayList`）做包装，所有方法都加 `synchronized` 锁，保证同一时间只有一个线程能访问列表。**（读写都加 `synchronized` 锁）**
- 特点：
  - 读写都加锁，读多写少场景性能也会受影响
  - 支持所有 List 操作，包括迭代器遍历，但遍历时需要手动加锁，否则可能出现并发修改异常
- **适用场景**：读写频率都不高，或者需要简单快速实现线程安全的场景。

### 2. `CopyOnWriteArrayList`

- **原理**：写时复制（Copy-On-Write）。每次修改（add/set/remove）都会复制一份新数组，在新数组上修改，修改完成后再把原数组的引用指向新数组；读操作直接访问原数组，完全无锁。**（读无锁，写时复制 + 锁）**
- 特点：
  - 读操作完全无锁，性能极高；写操作因为要复制数组，开销较大
  - 迭代器遍历的是修改前的数组快照，不会抛出并发修改异常
  - 不支持 `addIfAbsent` 等需要判断的复合操作，也不支持 null 元素（JDK 早期版本）
- **适用场景**：读操作远多于写操作的场景，比如配置列表、事件监听器列表。
...
答：String 不可变，适合常量和读多写少场景；StringBuilder 可变，线程不安全，但拼接性能最好；StringBuffer 可变，线程安全，因为方法大多带同步。
