...
**面试版本：**ThreadLocal 我一般理解成一种**线程隔离机制**，它不是通过加锁解决线程安全，而是给每个线程单独提供一份变量副本，这样线程之间互不影响。它的底层可以简单理解为每个线程对象里都维护了一个 ThreadLocalMap，ThreadLocal 自己作为 key，要存的值作为 value，所以同一个 ThreadLocal 在不同线程里拿到的是各自独立的数据。它很适合存一些线程内共享、线程间隔离的上下文信息，比如用户信息、请求链路、数据库连接这些，也能减少方法间一层层传参。它的优点是没有锁竞争，性能比较好，但要注意它不是用来做线程间共享的。实际使用里还有一个重点，就是在线程池场景下一定要及时 remove，不然线程复用时旧数据可能残留，严重了还可能引发内存泄漏
