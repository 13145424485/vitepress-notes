...
## **4. Redis 的数据结构有哪些**/ redis高级数据结构的使用场景**
...
### Redis 后续版本新增的数据类型应用场景：

- **BitMap（2.2 版新增）**：二值状态统计的场景，比如签到、判断用户登陆状态、连续签到用户总数等；
- **HyperLogLog（2.8 版新增）**：海量数据基数统计的场景，比如百万级网页 UV 计数等；
- **GEO（3.2 版新增）**：存储地理位置信息的场景，比如滴滴叫车；
- **Stream（5.0 版新增）**：消息队列，相比于基于 List 类型实现的消息队列，有两个特有的特性：自动生成全局唯一消息 ID，支持以消费组形式消费数据。

#### **Redis 数据结构，重点讲一下 ZSet**


ZSet 就是有序集合，你可以把它理解成“带分数的去重集合”。

它有两个特点：第一，元素不能重复；

第二，每个元素会绑定一个分数，Redis 会按照分数自动排序。

它很适合做**排行榜、延迟队列、按时间排序的列表、关注列表这类场景**。底层上，小数据量时会用紧凑结构，大一些时一般是跳表加哈希表，哈希表负责快速查成员，跳表负责有序范围查询。所以它的价值不是简单存一组值，而是**既能去重，又能排序，还能高效做范围查询**。

#### ZSet 的底层数据结构，查询的时间复杂度是多少？

Zset 类型的底层数据结构是由**压缩列表或跳表**实现的：

- 如果有序集合的**元素个数小于 128** 个，并且**每个元素的值小于 64 字节**时，Redis 会使用压缩列表作为 Zset 类型的底层数据结构，压缩列表的查找操作是顺序查找，时间复杂度为 O (n)
...
Redis 持久化主要有两种：**RDB 和 AOF**。RDB 是定时把某个时间点的数据做成快照，恢复快，文件也紧凑；AOF 是把**写命令追加记录下来，数据更完整。**

RDB 会不会丢数据？会。因为它是按时间点做快照，比如你 5 分钟做一次快照，那宕机时，最近这几分钟的数据就可能丢。AOF 相对更稳，尤其是每秒刷盘的策略，最多丢 1 秒左右数据。实际生产里很多时候是两种一起开，兼顾恢复速度和数据安全。**RDB 恢复快但可能丢一段时间的数据，AOF 更安全但文件更大、恢复更慢。**

### RDB 是怎样做的？

所谓的快照，就是记录某一个瞬间东西，比如当我们给风景拍照时，那一个瞬间的画面和信息就记录到了一张照片。

所以，RDB 快照就是**记录某一个瞬间的内存数据**，**记录的是实际数据**，而 **AOF 文件记录的是命令操作的日志，而不是实际的数据**。因此在 Redis 恢复数据时，RDB 恢复数据的效率会比 AOF 高些，因为直接将 RDB 文件读入内存就可以，不需要像 AOF 那样还需要额外执行操作命令的步骤才能恢复数据。

Redis 提供了两个命令来生成 RDB 文件，分别是 `save` 和 `bgsave`，他们的区别就在于是否在「主线程」里执行：

- 执行了 `save` 命令，就会在**主线程生成 RDB 文件**，由于和执行操作命令在同一个线程，所以如果写入 RDB 文件的时间太长，会阻塞主线程；
- 执行了 `bgsave` 命令，会创建**一个子进程来生成 RDB 文件**，这样可以避免主线程的阻塞，`bgsave` 命令可以在主进程的基础上，fork 一个子进程，子进程会共享主进程的代码段和数据段，相当于是在后台生成快照。

### AOF 的写入策略，按时间写入和每次都写入的区别，优缺点
...
### 你平常是怎么使用 RDB 和 AOF 的？（线上环境如何选择？）

我平时在生产环境会结合两者的优势使用：**开启 AOF 作为主持久化方式，保证数据安全，同时定时执行 RDB 快照做冷备和快速恢复**。



不会固定选择某一种，而是根据 RPO、RTO和 Redis 的业务定位决定。

如果 Redis 只是缓存，数据可以从数据库重新构建，可以关闭持久化，或者只保留 RDB 用于加快恢复。

如果允许丢失几分钟数据，并且更关注恢复速度和资源开销，可以使用 RDB。

如果希望故障时最多丢失约一秒数据，可以使用 AOF，并配置每秒刷盘。

如果 Redis 中保存的是比较重要的业务状态，通常会同时开启 RDB 和 AOF：AOF负责更小的数据丢失窗口，RDB负责备份、灾难恢复和较快的数据迁移。
...
- AOF 可以采取每秒同步一次数据或每次写操作都同步用来保证数据安全性
...
##  Redis 是单线程的，用 Redis 的时候需要注意哪些问题？


第一，不能把耗时命令放进去，比如大范围扫描、一次拉太多数据；

第二，要避免大 key，不然读写、删除、迁移都很重；

第三，Lua 脚本虽然原子，但也不能写太重，不然会阻塞其他请求；

第四，业务上要控制好热点 key，因为 Redis 虽快，但热点集中时单线程也会成为瓶颈。所以 Redis 单线程不是说不能高并发，而是要求你命令要短、小、快，不能把它当数据库随便用。

### Redis 的大 Key 会有什么问题？

（一个 Key 对应的 **value 特别大**，或者一个 Key 下面挂了**海量元素**，就叫大 Key。)
大 Key 最大的问题是会**把单次操作变重**。比如读一个大 key 会占用更多网络带宽和反序列化时间，删一个大 key 可能阻塞 Redis，迁移和主从同步时也会更慢，严重时会把整个实例拖慢。业务层面还容易**导致热点更集中**。所以我的原则是：value 不要做得过大，集合不要无上限增长，必要时分片、拆桶、分页或者只缓存最近 N 条。
...
缓存本身基于 CAP 理论**牺牲强一致性来换性能**，设置过期时间也得拿捏好长短；要解决数据不一致和删缓存异常，主要有两个方案：一是用消息队列做删缓存重试，失败了就重新尝试，成功再移除消息，不过会有点业务代码侵入；二是订阅 MySQL binlog，通过 Canal 采集数据变更日志，再异步删缓存，这种方式更解耦且能保证最终一致性。
...
- 网络状况：如果网络状况良好且稳定，分布式缓存能够更好地发挥其优势；如果网络状况较差或不稳定性，本地缓存的访问速度和稳定性可能更有优势。
- 业务特点：对于实时性要求较...（原文截断）
...
#  热门商品的大量请求直接访问 Redis，会产生什么问题

会产生热 Key 问题。同一个商品 Key 集中在一个 Redis 节点上，大量请求可能耗尽该节点的 CPU、网络带宽和连接数，造成响应变慢甚至超时。

如果库存采用先查询再更新，还可能出现并发覆盖和超卖。因此可以用本地缓存分担商品查询，库存则使用 Redis Lua 脚本原子扣减，并配合限流和消息队列削峰。

# 仅依靠本地缓存能解决热点查询和库存更新问题吗？

> 不能。本地缓存只能降低查询压力，无法解决库存更新问题。
>
> 因为多台应用服务器各自保存一份缓存，数据可能不一致，而且本地缓存无法保证全局原子扣减，可能造成超卖。
>
> 所以商品详情可以使用本地缓存，库存则要通过 Redis 原子扣减，再异步更新数据库。