...
#### 在这个项目中，计数部分，假如有多个不同的用户进行点赞和一部分用户进行重复点赞，那么是怎么进行判重的呢？

这个项目的点赞判重不是靠统计请求次数，而是把“某个用户是否给某个内容点过赞”存成 Redis **分片位图中的一个 bit**。用户点赞时，会先**根据 userId 计算出对应的分片和 bit 位**，然后通过 Lua 脚本原子执行 GETBIT + SETBIT：如果这个 bit 原来已经是 1，说明该用户之前点过赞，本次就是重复点赞，直接返回不变，不会再发计数事件；

只有当 bit 从 0 变成 1 时，才认为是一次新的有效点赞并触发计数 +1。

所以多个不同用户点赞会分别计数，而同一个用户重复点赞只会生效一次，并且因为用了 Lua 脚本，连并发场景下也能保证判重和计数的一致性。

**根据 userId 计算出对应的分片和 bit 位**
...
不同用户的 bit 不同，所以多个不同用户点赞会分别计数

同一个用户重复点赞，命中的还是同一个 bit，所以能判重。
...
#### 现在有一个场景，如果在同一个对话，假如一开始对话内容是关于小说的，现在用户要问到关于历史方面的对话，这个问题应该怎么解决？

当前这个项目里的 AI 问答，主要解决的其实不是“通用聊天”，而是“针对单篇知文内容做基于上下文的问答”。入口在 KnowPostRagController.java (line 24)，它暴露了 /{id}/qa/stream 接口，说明问答是绑定某一篇 postId 的。核心逻辑在 RagQueryService.java (line 33)：

- 先确保当前帖子已经建好索引。
- 再去向量库里做语义检索，召回和问题最相关的内容片段。
- 检索后又按 postId 二次过滤，避免别的帖子内容串进来，见 RagQueryService.java (line 65)。
- 然后把“当前问题 + 当前帖子召回到的上下文”拼成 prompt，让模型基于这些内容回答，并明确要求“只能依据提供的知文上下文回答，无法确定就说明不确定”，见 RagQueryService.java (line 42)。
- 最后用 SSE 流式返回结果。
