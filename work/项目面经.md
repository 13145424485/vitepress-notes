...
点赞和计数。难点是**高并发下既要防止重复点赞导致重复计数**，又不能每次都直接更新总数，否则写放大会很大。我的做法是：写路径先用 Redis Bitmap + Lua 做原子置位，保证幂等；成功后发 Kafka 增量事件；消费者先写 Redis 聚合桶，再定时刷到 SDS 计数快照；如果快照缺失或损坏，就从 Bitmap 分片按需重建。这样写快、读也快，代价是接受秒级最终一致。
